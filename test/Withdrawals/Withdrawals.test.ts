...
describe("Withdrawals", function () {
  ...
  describe("Upgrade", () => {
    ...
    it("should upgrade the contract if the user has upgrader role", async function () {
      ...
      await expect(upgrades.upgradeProxy(withdrawalsProxy, MockWithdrawals)).to
        .be.reverted;
  ...
  it("should correctly set the StakeTogether address", async function () {
    ...
    await expect(connect(withdrawals, owner).setStakeTogether(nullAddress)).to
      .be.reverted;

    // User1 tries to set the StakeTogether address to their own address - should fail
    await expect(connect(withdrawals, user1).setStakeTogether(user1.address)).to
      .be.reverted;
