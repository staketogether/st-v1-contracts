...
function checkConfigVariables() {
  ...
  if (!process.env.ETH_MAINNET_BRIDGE_ADDRESS) missingVariables.push('ETH_MAINNET_BRIDGE_ADDRESS')
  if (!process.env.ETH_MAINNET_DEPOSIT_ADDRESS) missingVariables.push('ETH_MAINNET_DEPOSIT_ADDRESS')
  if (!process.env.ETH_MAINNET_WITHDRAWAL_ADDRESS) missingVariables.push('ETH_MAINNET_WITHDRAWAL_ADDRESS')
...
export async function deploy() {
  ...
  const bridgeAddress = process.env.ETH_MAINNET_BRIDGE_ADDRESS as string
  const depositAddress = process.env.ETH_MAINNET_DEPOSIT_ADDRESS as string
  const withdrawalsCredentials = process.env.ETH_MAINNET_WITHDRAWAL_ADDRESS as string
...
async function verifyContracts(adapterProxy: string, adapterImplementation: string) {
  ...
  console.log(`npx hardhat verify --network eth-mainnet ${adapterProxy} &&`)
  console.log(`npx hardhat verify --network eth-mainnet ${adapterImplementation}`)
}
