...
export default async function configureOpSepolia() {
  ...
  const stakeTogether = await ethers.getContractAt('StakeTogether', process.env.OP_SEPOLIA_L2_STAKE_TOGETHER_ADDRESS as string)
...
function checkConfigVariables() {
  ...
  if (!process.env.OP_SEPOLIA_L2_STAKE_TOGETHER_ADDRESS) missingVariables.push('OP_SEPOLIA_L2_STAKE_TOGETHER_ADDRESS')
