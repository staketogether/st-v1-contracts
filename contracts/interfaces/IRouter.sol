...
interface IRouter {
  ...
  struct Report {
    uint256 epoch; // Beacon report epoch
    bytes32 merkleRoot; // Merkle root for airdrop
    uint256 profitAmount; // Profit generated by validators
    uint256 profitShares; // stpShares equivalent to profit
    uint256 lossAmount; // Slashing amount generated by validators
    uint256 withdrawAmount; // Ether for withdrawals
    uint256 withdrawRefundAmount; // Ether returned to pool
    uint256 routerExtraAmount; // Extra ether on router
    bytes32[] validatorsToRemove; // List of validators to be removed by validator oracles
  }
