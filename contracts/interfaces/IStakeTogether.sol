...
interface IStakeTogether {
  ...
  function transferRewardsShares(address _account, uint256 _sharesAmount) external;
  ...
  function estimateFeePercentage(
    FeeType _feeType,
    uint256 _amount
  ) external view returns (uint256[4] memory _shares, uint256[4] memory _amounts);
